...
class VAE_HD(nn.Module):
    ...
    def _init_encoder(self):
        self.encoder = nn.Sequential(
            nn.Conv2d(1, 16, kernel_size=9, stride=3, padding=3),
            nn.BatchNorm2d(16), nn.ReLU(), #nn.Dropout(p=0.1),
            nn.Conv2d(16, 32, kernel_size=7, stride=3, padding=2),
    ...
    def _init_decoder(self):
        self.decoder = nn.Sequential(
            ...
            nn.ConvTranspose3d(64, 32, kernel_size=5, stride=3, padding=1),
            nn.BatchNorm3d(32), nn.ReLU(), #nn.Dropout(p=0.1),
            nn.ConvTranspose3d(32, 16, kernel_size=5, stride=3, padding=1),
            nn.BatchNorm3d(16), nn.ReLU(), #nn.Dropout(p=0.1),
            nn.ConvTranspose3d(16, 8, kernel_size=7, stride=3, padding=2),
            nn.BatchNorm3d(8), nn.ReLU(), #nn.Dropout(p=0.1),
            nn.ConvTranspose3d(8, 1, kernel_size=9, padding=4 ),
            nn.Sigmoid()
            )
...
class VAE_MD(nn.Module):
    ...
    def _init_decoder(self):
        self.decoder = nn.Sequential(
            ...
            nn.ConvTranspose3d(32, 16, kernel_size=5, stride=3, padding=1),
            nn.BatchNorm3d(16), nn.ReLU(), #nn.Dropout(p=0.1),
            nn.ConvTranspose3d(16, 8, kernel_size=5, stride=2, padding=2),
            nn.BatchNorm3d(8), nn.ReLU(), #nn.Dropout(p=0.1),
            nn.ConvTranspose3d(8, 1, kernel_size=7, padding=3 ),
            nn.Sigmoid()
            )
